...
class Instrument:
    ...
    def __init__(self, config: dict | None = None):
        ...
        if not isinstance(self.name, str) or not self.name:
            msg = (
                "Class property name must be defined as a non-empty string for {}".format(
                    self.__class__
                )
            )
            logger.error(msg)
        ...
        if not isinstance(self.desc, str) or not self.desc:
            msg = (
                "Class property desc must be defined as a non-empty string for {}".format(
                    self.__class__
                )
            )
            logger.error(msg)
    ...
    def read_packages(self):
        ...
        for nr, source_file in enumerate(source_files):
            ...
            #     )
            #     post_event('on_resource_rejected', dict(
            #           instrument=self.name,
            #           path=Path(self.source_directory, source_file))
            #     )
            #     continue
            #
            ...
            #         instrument=self.name,
            #         resource=resource,
            #         path=resource.absolute_source_path
            #     )
            # )
        post_event(
            "on_progress",
            dict(instrument=self.name, msg="Done reading files", percentage=100),
        )
    ...
    def _add_config_attributes_to_resource(self, resource):
        """Adds config attributes given to the instrument class to the given resource.
        If value is given in the config attributes this value will replace any old value
        in the resource attributes. If value is missing the old resource attribute value
        will be kept"""
    ...
    def packages(self) -> PackageCollection:
        if not isinstance(self._packages, PackageCollection):
            msg = (
                f"Packages has not yet been extracted for {self.__class__.__name__}. "
                f"Make sure to call the read_packages method before "
                f"trying to access packages."
            )
            logger.error(msg)
            raise exceptions.PackagesNotExtractedError(msg)
