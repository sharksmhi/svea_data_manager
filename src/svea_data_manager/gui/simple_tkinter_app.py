...
class App(tk.Tk):
    ...
    def _build_frame_paths(self):
        ...
        tk.Label(frame, textvariable=self._stringvar_config).grid(
        ...
        tk.Label(frame, textvariable=self._stringvar_root).grid(
            row=r, column=1, **grid, sticky="w"
        )

        r += 1
        self._button_run_all = tk.Button(
    ...
    def _build_frame_config(self):
        ...
        for inst in self._config:
            ...
            tk.Label(frame, text="-" * line_length).grid(
            ...
            for key, item in self._config[inst].items():
                if key == "source_directory":
                    self._buttons_path[inst_lower] = tk.Button(
                        frame,
                        text=key,
                    ...
                    tk.Label(frame, text=str(key)).grid(
                    ...
                    for attr, value in item.items():
                        if not value:
                            value = ""
                        tk.Label(attr_frame, text=attr).grid(
                    ...
                    tk.Label(frame, text=str(item)).grid(
                        row=r, column=1, **gridl, sticky="w"
                    )
                grid_configure(frame, nr_columns=2, nr_rows=r + 1)

                r += 1
            self._buttons_run[inst_lower] = tk.Button(
            ...
            conf_r += 1
        tk.Label(self._inner_config_frame, text="-" * line_length).grid(
    ...
    def _run_all_instruments(self):
        ...
        try:
            ...
            nr_copied_str = "\n".join(
                [
                    f"{inst}: {nr}"
                    for inst, nr in self._logger.get_nr_files_copied().items()
                ]
            )
            nr_not_copied_str = "\n".join(
                [
            ...
            msg = (
                f"Hanteringen är klar för samtliga instrument. "
    ...
    def _run_instrument(self, inst, show_message=False):
        ...
        try:
            ...
            msg = (
                f"Hanteringen är klar för instrument: {inst.upper()}. \n\n"
