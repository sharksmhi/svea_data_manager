...
class DataRaw:
    ...
    def copy_keys_to_data_directory(
        self, keys: list[str], directory: pathlib.Path | str
    ) -> int:
        nr = 0
        for nr, (source_path, rel_target_path) in enumerate(
            self.get_paths_for_keys(*keys)
        ):
