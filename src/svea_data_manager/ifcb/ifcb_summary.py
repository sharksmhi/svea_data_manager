...
class IFCBSummaryFile:
        ...
        for root, dirs, files in os.walk(directory, topdown=False):
            ...
            for name in files:
                ...
                if nr_loaded == nr_in_scope:
                    logger.info("Found all hdr files in scope! Stop checking files.")
                    return
    ...
    #     header = np.array(
    #         [FILE_PAR, 'datetime', 'lat', 'lon', VOLUME_PAR] + list(self._class_data)
    #     )
    #     data = np.array(
    #       [
    #           self._file_data, self._datetime_data, self._lat_data, self._lon_data,
    #           self._volume_data
    ...
    def _combine_data(self):
        ...
        self._all_data.append(
            ["id", "datetime", "lat", "lon", "taxon", "parameter", "value"]
        )
        for _id in self._id_list:
            common_line = [
                _id,
                self._datetime_data[_id],
                self._get_lat(_id),
                self._get_lon(_id),
            ]
            for taxon in self._class_list:
                ...
                self._all_data.append(
                    [*common_line[:], taxon, "classifier_name", classifier_name]
                )
                self._all_data.append(
                    [*common_line[:], taxon, "classifier_run_date", date]
                )

    def _load_mat_summary_data(self):
        self._mat_summary_data = scipy.io.loadmat(
            self._mat_summary_path, simplify_cells=True
        )

        self._id_list = [
            file["name"].split(".")[0] for file in self._mat_summary_data[FILE_PAR]
        ]
...
def create_summary_file(
    ...
    **kwargs,
    ...
    if hdr_root_directory and pathlib.Path(hdr_root_directory).exists():
        ...
        logger.warning(
            "No hdr_root_directory given to add positional data to ifcb summary file"
        )
    ...
    if mat_root_directory and pathlib.Path(mat_root_directory).exists():
        ...
        logger.warning(
            "No mat_root_directory given to add positional data to ifcb summary file"
        )
