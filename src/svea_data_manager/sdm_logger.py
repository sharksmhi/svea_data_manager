...
class SDMLogger:
    ...
    def _on_resource_added(self, data):
        ...
        self._callbacks["resources_added"][data["instrument"].upper()].append(
            f"{data['path']} - {data['resource'].__class__.__name__}"
        )

    def _on_resource_rejected(self, data):
        self._callbacks["resources_rejected"].setdefault(data["instrument"].upper(), [])
        self._callbacks["resources_rejected"][data["instrument"].upper()].append(
            data["path"]
        )

    def _on_target_path_exists(self, data):
        self._callbacks["target_path_exists"].setdefault(data["instrument"].upper(), [])
        self._callbacks["target_path_exists"][data["instrument"].upper()].append(
            data["path"]
        )

    def on_transform_add_file(self, data):
        self._callbacks["transform_added_files"].setdefault(
            data["instrument"].upper(), []
        )
        self._callbacks["transform_added_files"][data["instrument"].upper()].append(
            data["name"]
        )

    def _on_file_copied(self, data):
        self._callbacks["files_copied"].setdefault(data["instrument"].upper(), [])
        self._callbacks["files_copied"][data["instrument"].upper()].append(
            data["target_path"]
        )

    def _on_svn_prepared(self, data):
        self._callbacks["svn_prepared"].setdefault(data["instrument"].upper(), [])
        self._callbacks["svn_prepared"][data["instrument"].upper()].append(
            data["target_path"]
        )
    ...
    def get_nr_resources_added(self, instrument=None):
        if instrument:
            return self._get_len(
                self._callbacks["resources_added"].get(instrument.upper())
            )
        else:
            return dict(
                (key, len(values))
                for key, values in self._callbacks["resources_added"].items()
            )

    def get_nr_resources_rejected(self, instrument=None):
        if instrument:
            return self._get_len(
                self._callbacks["resources_rejected"].get(instrument.upper())
            )
        else:
            return dict(
                (key, len(values))
                for key, values in self._callbacks["resources_rejected"].items()
            )

    def get_nr_target_path_exists(self, instrument=None):
        if instrument:
            return self._get_len(
                self._callbacks["target_path_exists"].get(instrument.upper())
            )
        else:
            return dict(
                (key, len(values))
                for key, values in self._callbacks["target_path_exists"].items()
            )

    def get_nr_transform_added_files(self, instrument=None):
        if instrument:
            return self._get_len(
                self._callbacks["transform_added_files"].get(instrument.upper())
            )
        else:
            return dict(
                (key, len(values))
                for key, values in self._callbacks["transform_added_files"].items()
            )
    ...
    def get_nr_files_copied(self, instrument=None):
        if instrument:
            ...
            return dict(
                (key, len(values))
                for key, values in self._callbacks["files_copied"].items()
            )
    ...
    def get_nr_svn_prepared(self, instrument=None):
        if instrument:
            ...
            return dict(
                (key, len(values))
                for key, values in self._callbacks["svn_prepared"].items()
            )
    ...
    def write_reports(self, directory=None):
        ...
        root_directory = Path(
            self._report_directory, datetime.datetime.now().strftime("%Y%m%d_%H%M")
        )
        for callback, info in self._callbacks.items():
            if isinstance(info, list):
                ...
                for inst, values in info.items():
                    path = Path(
                        root_directory, inst, f"{callback}_{len(values)}_files.txt"
                    )
                    path.parent.mkdir(parents=True, exist_ok=True)
                    with open(path, "w") as fid:
